...
public class Main {
    public static void main(String[] args){
        Scanner scanner = new Scanner(Main.class.getResourceAsStream("day03-2.txt"));
        //read the first line
        ...
        for(Point point:points){
            if(point.getSteps()<minSteps)
                minSteps = point.getSteps();
        }

        System.out.println("Result: " + minSteps);
    }
    ...
    private static MultiLine createLine(String[] inputs){
        ...
        for(String input:inputs){
            ...
            switch (opcode){
                case "U":
                    multiLine.addLine(lastPoint.getX(), lastPoint.getY()+distance);
                    break;
                case "D":
                    multiLine.addLine(lastPoint.getX(), lastPoint.getY()-distance);
                    break;
                case "L":
                    multiLine.addLine(lastPoint.getX()+distance, lastPoint.getY());
                    break;
                case "R":
                    multiLine.addLine(lastPoint.getX()-distance, lastPoint.getY());
                    break;
