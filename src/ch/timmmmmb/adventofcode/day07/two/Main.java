...
public class Main {
    public static void main(String[] args){
        Scanner scanner = new Scanner(Main.class.getResourceAsStream("day07-2.txt"));
        //read the first line
        ...
        int max = 0;
        int[] elements = {5,6,7,8,9};    // input array
        ...
        for(int[] combination: combinations){
            ...
            for(int j = 0; j<combination.length; j++){
                if(j == 0){
                    value = calculateValue(intinputs.clone(), combination, value, j);
                }else{
